...
*/
    ...
    function inputsEmptyLogin($index, $password){
     $value = null;
    ...
    function inputs1EmptyLogin($identity, $password){
        $value = null;
...
*/
    ...
    function indexInvalid($index){
        $value = null;
        if(!preg_match("/^[a-zA-Z\d\._-]+[a-zA-Z\d_-]$/", $index)){
            $value = true; 
        }
        else{
            $value = false;
    ...
    function identityInvalid($identity){
        $value = null;
        if(!preg_match("/^\d+(?:[A-Za-z]|)$/", $identity)){
            $value = true; 
        }
        else{
            $value = false;
